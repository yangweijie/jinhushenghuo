...
var
    ...
    defaults = {
        ...
        "modalClose"  : false,     // 点击模态背景是否关闭弹出层
...
ThinkBox = function(element, options){
    ...
    includeCss("/skin/" + options.style + "/style.css", function(){
        ...
        //窗口大小改变后重设位置
        options.resize && _win.on("resize.thinkbox-" + self.key,function(){self.resetLocate()});
...
ThinkBox.prototype = {
    ...
    "setStatus" : function(content,status){
       //数字type转换为字符串type
        switch(status){
            case 0: status = "error"; break;
            case 1: status = "success"; break;
        }
        $(".thinkbox-tools", this.box).append("<span class=\"thinkbox-status-" + status + "\">" + content + "</span>");
        return this;
    },
...
$.extend($.thinkbox, {
    ...
    "notice": function(msg,opt){
        ...
        var html = $("<div/>").addClass("thinkbox-notice").html(msg);
        return $.thinkbox(html, options);
    },
